...
for finset = 1:bodyDims(7)
    ...
    for AngleAttack = deg2rad((minAngleAttack:DeltaAngleAttack:maxAngleAttack))
        ...
        for Mach = minMach+deltaMach:deltaMach:1-deltaMach
            ...
            p_inf=101325*(1+(gamma-1/2)*Mach^2)^(-gamma/(gamma-1));
            q_inf=(gamma/2)*p_inf*(Mach)^2;

            D(i,j,finset)=c_D*q_inf*sTemp;
            L(i,j,finset)=c_L*q_inf*sTemp;
...
rho_SSL = rho_inf;
temperature_SSL = 288.15; %Kelvin

CDOindex=1;
for Mach = 0+deltaMach:deltaMach:maxMach
    %Isentropic relations
    rho(CDOindex) = rho_SSL*(1 + (gamma-1)/2*Mach^2)^(-1/(gamma-1)); % kg/m^3
    temperature(CDOindex) = temperature_SSL*(1 + (gamma-1)/2*Mach^2)^(-1); % Kelvin
    % Sutherland for viscosity
    mu(CDOindex) = mu_inf*(temperature(CDOindex)/temperature_SSL)^(3/2)*(temperature_SSL+110)/(temperature(CDOindex)+110);
    ...
    ReBody = (rho_inf*Mach*a*TubeLength)/mu(CDOindex);
